...
// Upsell recommendations based on what was found during pre-service checklist
const UPSELL_ITEMS = {
...
module.exports = async function handler(req, res) {
  ...
  if (!bookingId || !criticalItems?.length) {
  ...
  const upsellRows = criticalItems
    ...
    .map(item => {
      ...
      return '<tr><td style="padding:10px 12px;border-bottom:1px solid #E5E7EB"><strong>' + esc(u.label) + '</strong><br><span style="font-size:12px;color:#6B7280">' + esc(u.desc) + '</span></td><td style="padding:10px 12px;border-bottom:1px solid #E5E7EB;text-align:right;font-weight:700;color:#0A58CA">$' + u.price + '</td></tr>';
    }).join('');
  ...
  const emailHtml = `
    <div style="font-family:system-ui,sans-serif;max-width:560px;margin:0 auto">
      ...
      <div style="padding:32px 24px">
        <h2 style="font-size:20px;color:#0D1F3C">Hi ${esc(name)}, your mechanic found some things that need attention 🔧</h2>
        <p style="color:#374151;line-height:1.7;margin-top:12px">
          During your pre-service inspection, we identified a few items that should be addressed soon for safety and to avoid more costly repairs later.
        </p>
        <table style="width:100%;border-collapse:collapse;margin:20px 0;border:1px solid #E5E7EB;border-radius:8px;overflow:hidden">
          <thead><tr style="background:#F7F8FA"><th style="padding:10px 12px;text-align:left;font-size:13px;color:#6B7280">Recommended Service</th><th style="padding:10px 12px;text-align:right;font-size:13px;color:#6B7280">Price</th></tr></thead>
  ...
  try {
    const resp = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: email,
        subject: `Your mechanic found items that need attention — ${booking.service_type || 'your service'}`,
        html: emailHtml
      })
    });
    ...
    console.error('Upsell email error:', e);
