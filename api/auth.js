...
async function handleMechanicJobs(req, res) {
  ...
  const jobsResp = await fetch(
    `${SUPABASE_URL}/rest/v1/bookings?select=${cols}&order=scheduled_date.asc,scheduled_time.asc`,
...
export default async function handler(req, res) {
  ...
  const rateMax = role.startsWith('mechanic-') ? 30 : (role === 'public-track' || role === 'public-booking-list' || role.startsWith('client-')) ? 20 : 5;
