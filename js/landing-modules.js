...
(async () => {
  ...
  try {
    const { GrowthBook } =
      await import('https://cdn.jsdelivr.net/npm/@growthbook/growthbook/dist/bundles/esm.min.js');
...
// El diccionario ANTES de traducir la pagina por primera vez. Sin esto un
// visitante en espanol o chino ve la landing en ingles y la ve cambiar un
// instante despues. Para un visitante en ingles no espera nada.
...
const LANG_CARET =
  '<svg class="nav-lang-caret" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" aria-hidden="true"><polyline points="6 9 12 15 18 9"/></svg>';
const LANG_CHECK =
  '<svg class="nav-lang-check" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="20 6 9 17 4 12"/></svg>';
...
function renderLangSwitcher() {
  ...
  wrap.innerHTML =
    ...
    LANGUAGES.map(function (l) {
      return (
        '<button type="button" class="nav-lang-option" role="option" data-lang="' +
        l.code +
        '"' +
        ' aria-selected="' +
        (l.code === current.code) +
  ...
  function openMenu(index) {
    ...
    const start =
      typeof index === 'number'
        ? index
        : Math.max(0, options.indexOf(wrap.querySelector('[aria-selected="true"]')));
    options[start].focus();
  }
