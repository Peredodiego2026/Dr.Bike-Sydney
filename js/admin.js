...
function applyDarkModeInline(){
  ...
  document.querySelectorAll('[style]').forEach(el => {
    const s = el.style;
    if(isDark){
      if(s.color === 'var(--navy)' || el.getAttribute('style')?.includes('color:var(--navy)') || el.getAttribute('style')?.includes('color: var(--navy)')){
        el.style.setProperty('color','#F2F2F7','important');
      }
      if(s.color === 'var(--mgray)' || el.getAttribute('style')?.includes('color:var(--mgray)') || el.getAttribute('style')?.includes('color: var(--mgray)')){
        el.style.setProperty('color','#8E8E93','important');
      }
      if(el.getAttribute('style')?.includes('background:#fff') || el.getAttribute('style')?.includes('background: #fff') || el.getAttribute('style')?.includes('background:var(--off)') || el.getAttribute('style')?.includes('background:var(--off)')){
        if(!el.getAttribute('style')?.includes('rgba')){
          el.style.setProperty('background','#242426','important');
        }
      }
      if(el.getAttribute('style')?.includes('border-color:var(--border)') || el.getAttribute('style')?.includes('border:1px solid var(--border)') || el.getAttribute('style')?.includes('border: 1px solid #E5E7EB')){
        el.style.setProperty('border-color','#38383A','important');
      }
      if(el.getAttribute('style')?.includes('color:var(--mgray)') || el.getAttribute('style')?.includes('color: #6B7280')){
        el.style.setProperty('color','#8E8E93','important');
      }
      if(el.getAttribute('style')?.includes('color:var(--navy)') || el.getAttribute('style')?.includes('color:#0d1f3c')){
        el.style.setProperty('color','#F2F2F7','important');
      }
    } else {
...
function toggleAdminTheme(){
  ...
  document.getElementById('theme-btn-admin').textContent = next === 'dark' ? '☀️' : '🌙';
  setTimeout(applyDarkModeInline, 50);
}
// Init theme button icon
(function(){
  const t = localStorage.getItem('drbike-theme') || (window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light');
  document.documentElement.setAttribute('data-theme', t);
  const btn = document.getElementById('theme-btn-admin');
  if(btn) btn.textContent = t === 'dark' ? '☀️' : '🌙';
  const db = document.getElementById('admin-date-badge');
  if(db) db.textContent = new Date().toLocaleDateString('en-AU',{weekday:'short',day:'numeric',month:'short',year:'numeric'});
  setTimeout(applyDarkModeInline, 100);
})();
...
// ── FINANCE ───────────────────────────────────────────────────────────────────
const FIXED_COSTS = { payroll: 0, fleet: 960, insurance: 360, marketing: 400, software: 120, other: 360 };
...
async function loadTriggers() {
  try {
    const { data } = await sb.from('van_zones').select('suburb,postcode')
      .eq('van_number', 0)
      .in('suburb', TRIGGER_KEYS.map(k => '__trig_' + k + '__'));
    const map = {};
    (data||[]).forEach(r => { map[r.suburb] = r.postcode; });
  ...
  } catch(e) { console.warn('loadTriggers:', e); }
}
...
async function sendTestSMS() {
  ...
  try {
    ...
    const { data } = await sb.from('escalation_contacts').select('phone').eq('role','manager').limit(1).single();
    if(!data?.phone) { showToast('No manager number found — add one in Notification Numbers'); btn.textContent = '📱 Send test SMS to your number'; btn.disabled=false; return; }
    const res = await fetch('/api/send-sms', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ to: data.phone, name: 'Diego', service: 'Tune-Up', address: 'Newtown', price: 109, type: 'test', bookingId: 'TEST-001' })
    });
    const json = await res.json();
    if(res.ok) { showToast('✓ Test SMS sent to ' + data.phone); }
    else { showToast('SMS failed: ' + (json.error || 'unknown error')); }
...
async function loadFinance() {
  ...
  const plRows = [
    ...
    { label:'Net profit', val: netProfit, bold: true, total: true, color: netProfit>=0?'var(--green)':'var(--red)' },
  ];

  const periodStr = view==='month' ? new Date(year,month-1,1).toLocaleString('en-AU',{month:'long',year:'numeric'})
    : view==='quarter' ? `Q${Math.ceil(month/3)} ${year}` : `FY ${year}`;

  document.getElementById('fin-pl-period').textContent = periodStr;
  document.getElementById('fin-pl-rows').innerHTML = plRows.map(r=>`
  ...
  document.getElementById('fin-chart').innerHTML = days.length ? days.map(d=>{
    const v = dailyMap[d];
  ...
  }).join('') : '<div style="color:var(--mgray);font-size:13px;margin:auto">No completed jobs in this period</div>';
  ...
  document.getElementById('fin-tx-body').innerHTML = jobs.length ? jobs.map(j=>{
    const price = j.service_price||0;
    const jGst  = Math.round(price/11);
    const jNet  = price - jGst;
    const name  = j.client_name || j.profiles?.full_name || j.profiles?.email?.split('@')[0] || 'Client';
    return `<tr>
  ...
  }).join('') : '<tr><td colspan="7"><div style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:48px 24px;gap:10px"><svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="var(--border)" stroke-width="1.5"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg><div style="font-size:14px;font-weight:600;color:var(--mgray)">No transactions yet</div><div style="font-size:12px;color:var(--mgray);opacity:0.7">Completed jobs will appear here</div></div></td></tr>';

  // Store for export
  window._finData = { jobs, revenue, gst, netRevenue, grossProfit, netProfit, margin, jobCount, avgJob, periodStr, dateFrom, dateTo };
}
...
function exportFinanceCSV() {
  ...
  const rows = [
    ...
    ...d.jobs.map(j => {
      ...
      return [j.scheduled_date, j.profiles?.full_name||j.profiles?.email||'', j.service_name||'', price, gst, price-gst, 'Completed'];
    }),
    [],
    ['SUMMARY','','','','','',''],
    ['Period',d.periodStr,'','','','',''],
...
function exportFinancePDF() {
  const d = window._finData;
  if (!d) { toast('Load finance data first'); return; }

  // Service breakdown
  const svcMap = {};
  (d.jobs||[]).forEach(j => { const k = j.service_name||'Other'; svcMap[k] = (svcMap[k]||0) + (j.service_price||0); });
  ...
  <body><div class="page">
    ...
    <table>
      ...
      <tbody>${(d.jobs||[]).map(j=>{
        const p=j.service_price||0, g=Math.round(p/11);
        return '<tr><td>'+(j.scheduled_date||'—')+'</td><td class="bold">'+(j.profiles?.full_name||'Client')+'</td><td>'+(j.service_name||'—')+'</td><td>Van '+(j.van_number||1)+'</td><td class="blue">$'+p+'</td><td>$'+g+'</td><td>$'+(p-g)+'</td></tr>';
      }).join('')}
      ...
      <div style="display:flex;flex-direction:column;gap:14px">
        ...
        <div>
          ...
          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px" id="block-slots">
            ${['8:00','8:30','9:00','9:30','10:00','10:30','11:00','11:30','12:00','12:30','13:00','13:30','14:00','14:30','15:00','15:30','16:00','16:30'].map(t=>
...
function selectAllSlots(sel) {
  document.querySelectorAll('#block-slots input[type=checkbox]').forEach(cb => cb.checked = sel);
}
...
async function saveBlocks() {
  ...
  if(!slots.length){ showToast('Select at least one slot'); return; }

  const rows = slots.map(time => ({
    date, time_slot: time, van_number: van || null, blocked: true, reason: reason || null
  }));

  const { error } = await sb.from('availability').upsert(rows, { onConflict: 'date,time_slot,van_number' });
  if(error){ showToast('Error: ' + error.message); return; }
  
  document.getElementById('block-modal').remove();
  showToast(`✅ ${slots.length} slot${slots.length>1?'s':''} blocked for ${new Date(date+'T00:00:00').toLocaleDateString('en-AU',{weekday:'short',day:'numeric',month:'short'})}`);
  loadCalendar();
}
...
async function unblockDate() {
  ...
  document.getElementById('block-modal').remove();
  showToast(`✅ All slots unblocked for ${new Date(date+'T00:00:00').toLocaleDateString('en-AU',{weekday:'short',day:'numeric',month:'short'})}`);
  loadCalendar();
}
...
async function sendBroadcastPush() {
  ...
  try {
    ...
    const { data: profiles } = await sb.from('profiles').select('id').eq('role','client').not('push_subscription','is',null);
    if(!profiles?.length){ res.innerHTML = '<span style="color:var(--mgray)">No subscribed clients yet.</span>'; return; }
    ...
    await Promise.allSettled((profiles||[]).map(async p => {
      try {
        await fetch('/api/send-push', {
          ...
          body: JSON.stringify({ clientId: p.id, title, body, url, tag:'broadcast-'+Date.now() })
        });
        sent++;
      } catch(e){}
    }));
...
async function loadReferralLeaderboard() {
  ...
  if(!data?.length){
    el.innerHTML = '<div style="text-align:center;padding:20px;color:var(--mgray);font-size:13px">No referrals yet — share the app!</div>';
    return;
  }
...
function showToast(msg){
  ...
  if(!t){
    ...
    t.id = 'admin-toast';
    t.style.cssText = 'position:fixed;bottom:24px;right:24px;background:#0D1F3C;color:#fff;padding:12px 20px;border-radius:10px;font-size:13px;font-weight:500;z-index:9999;opacity:0;transition:opacity .3s;font-family:Inter,sans-serif';
...
async function loadCoupons(){
  ...
  if(!grid) return;
  grid.innerHTML = '<div style="text-align:center;padding:48px;color:var(--mgray);grid-column:1/-1;font-size:13px">Loading...</div>';
  ...
  try {
    ...
    if(!data?.length){
      grid.innerHTML = '<div style="text-align:center;padding:48px;color:var(--mgray);grid-column:1/-1;font-size:13px">No codes yet — create your first one above! 🎟️</div>';
      return;
    }
    ...
    grid.innerHTML = data.map(c => {
      ...
      const expDisplay = c.expires_at ? new Date(c.expires_at).toLocaleDateString('en-AU',{day:'numeric',month:'short',year:'numeric'}) : 'No expiry';
...
function toast(msg){
  let t = document.getElementById('admin-toast');
  if(!t){ t=document.createElement('div'); t.id='admin-toast'; t.style.cssText='position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:#0D1F3C;color:#fff;padding:10px 20px;border-radius:10px;font-size:13px;font-weight:500;z-index:9999;opacity:0;transition:opacity .2s;pointer-events:none;font-family:Inter,sans-serif'; document.body.appendChild(t); }
...
async function saveCoupon(){
  ...
  if(!code){ toast('⚠️ Please enter a code'); savingCoupon=false; return; }
  if(!value || isNaN(value)){ toast('⚠️ Please enter a discount value'); savingCoupon=false; return; }

  const btn = document.getElementById('c-save-btn');
  if(btn){ btn.textContent='Creating…'; btn.disabled=true; }

  try {
    const insertData = { code, discount_type: type, discount_value: value, active: true, uses_count: 0 };
...
async function sendReminders(){
  ...
  try {
    const res = await fetch('/api/send-reminders', { method:'POST', headers:{'Content-Type':'application/json'}, body:'{}' });
    const data = await res.json();
    result.style.display = 'block';
    result.style.color = res.ok ? 'var(--green)' : 'var(--red)';
    result.textContent = res.ok ? `✅ ${data.message}` : `❌ Error: ${data.error}`;
  } catch(e){ result.style.display='block'; result.textContent='❌ '+e.message; }
  btn.textContent = '🚲 Send reminders now'; btn.disabled = false;
}
...
function checkAdminAuth() {
  if(sessionStorage.getItem('drbike-admin-token')) return true;
  const overlay = document.createElement('div');
  overlay.id = 'admin-login-overlay';
  overlay.style.cssText = 'position:fixed;inset:0;background:#0D1F3C;z-index:99999;display:flex;align-items:center;justify-content:center;font-family:Inter,sans-serif';
  overlay.innerHTML = `
    <div style="background:#fff;border-radius:20px;padding:40px 36px;width:100%;max-width:360px;text-align:center;box-shadow:0 20px 60px rgba(0,0,0,0.3)">
      <div style="width:56px;height:56px;background:#1848C8;border-radius:14px;display:flex;align-items:center;justify-content:center;margin:0 auto 20px;font-size:28px">🚲</div>
...
async function submitAdminLogin() {
  ...
  try {
    ...
    if(errEl) { errEl.textContent = e.message || 'Invalid credentials'; errEl.style.display='block'; }
    const inp = document.getElementById('admin-pass-inp');
    if(inp) { inp.value=''; inp.style.borderColor='#DC2626'; inp.focus(); }
...
async function loadDashboard(){
  ...
  const avgOrder = completedMonth.length ? Math.round(completedMonth.reduce((s,b)=>s+(b.service_price||0),0)/completedMonth.length) : 0;
  const cancelRate = (monthJobs||[]).length ? Math.round(((monthJobs||[]).filter(b=>b.status==='cancelled').length/(monthJobs||[]).length)*100) : 0;

  const kpis = document.querySelectorAll('#page-dashboard .kpi-value');
  if(kpis[0]){ kpis[0].textContent = '$'+todayRev.toLocaleString(); kpis[0].nextElementSibling.textContent = (todayJobs||[]).length+' jobs today · $'+Math.round(todayRev/Math.max((todayJobs||[]).length,1))+' avg'; }
  ...
  if(tbody && recentBookings){
    const stClass = {confirmed:'confirmed',pending:'pending',enroute:'enroute',completed:'completed',cancelled:'cancelled'};
    tbody.innerHTML = recentBookings.map(b=>{
      const name = b.client_name || b.profiles?.full_name || 'Client';
      const initials = name.split(' ').map(n=>n[0]).join('').slice(0,2).toUpperCase();
      const st = b.status||'pending';
      const stColors = {pending:'#D97706',confirmed:'#1848C8',enroute:'#059669',completed:'#6B7280',cancelled:'#DC2626'};
      const stBg = {pending:'#FEF3C7',confirmed:'#EEF3FC',enroute:'#ECFDF5',completed:'#F3F4F6',cancelled:'#FEF2F2'};
      const stLabel = {pending:'Pending',confirmed:'Confirmed',enroute:'En route',completed:'Completed',cancelled:'Cancelled'};
    ...
    }).join('') || '<tr><td colspan="6" style="text-align:center;color:var(--mgray);padding:24px">No bookings yet — ready to go!</td></tr>';
  }
  ...
  if(todaySub) todaySub.textContent = `${todayDate} · ${(todayJobs||[]).length} job${(todayJobs||[]).length!==1?'s':''}`;

  const todayTbody = document.getElementById('dash-today-tbody');
  if(todayTbody){
    const stColors2={pending:'#D97706',confirmed:'#1848C8',enroute:'#059669',completed:'#6B7280',cancelled:'#DC2626'};
    const stBg2={pending:'#FEF3C7',confirmed:'#EEF3FC',enroute:'#ECFDF5',completed:'#F3F4F6',cancelled:'#FEF2F2'};
    const stLabel2={pending:'Pending',confirmed:'Confirmed',enroute:'En route',completed:'Completed',cancelled:'Cancelled'};
    if(todayJobs && todayJobs.length > 0){
      const sorted = [...todayJobs].sort((a,b)=>(a.scheduled_time||'').localeCompare(b.scheduled_time||''));
      todayTbody.innerHTML = sorted.map(b=>{
        const st=b.status||'pending';
        const vanNum=b.van_number||1;
  ...
  if(schList){
    const upcoming = (todayJobs||[]).filter(b=>b.status!=='completed'&&b.status!=='cancelled')
      .sort((a,b)=>(a.scheduled_time||'').localeCompare(b.scheduled_time||''));
    if(schSub) schSub.textContent = `${upcoming.length} upcoming today`;
    const stDotColors={pending:'#F59E0B',confirmed:'#1848C8',enroute:'#059669',completed:'#6B7280',cancelled:'#DC2626'};
    if(upcoming.length > 0){
      schList.innerHTML = upcoming.map(b=>`
        <div class="sch-item">
...
function applyBookingFilters(){
  ...
  let filtered = allBookings.filter(b=>{
    ...
    if(search){
      const name = (b.client_name || b.profiles?.full_name || b.profiles?.email || '').toLowerCase();
...
function resetBookingFilters(){
  ['bk-f-from','bk-f-to','bk-f-search'].forEach(id=>{ const el=document.getElementById(id); if(el) el.value=''; });
  ['bk-f-van','bk-f-status'].forEach(id=>{ const el=document.getElementById(id); if(el) el.value=''; });
  applyBookingFilters();
}
...
function renderBookingsTable(data){
  ...
  if(el('bk-confirmed')) el('bk-confirmed').textContent = data.filter(b=>b.status==='confirmed').length;
  if(el('bk-pending')) el('bk-pending').textContent = data.filter(b=>b.status==='pending').length;
  if(el('bk-revenue')) el('bk-revenue').textContent = '$'+revenue.toLocaleString();
  if(el('bk-sub')) el('bk-sub').textContent = `${data.length} booking${data.length!==1?'s':''} · filtered`;

  if(!data.length){
    tbody.innerHTML = '<tr><td colspan="8"><div style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:48px 24px;gap:10px"><svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="var(--border)" stroke-width="1.5"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg><div style="font-size:14px;font-weight:600;color:var(--mgray)">No bookings found</div><div style="font-size:12px;color:var(--mgray);opacity:0.7">Try adjusting your filters</div></div></td></tr>';
    return;
  }

  tbody.innerHTML = data.map(b=>{
    const name = b.client_name || b.profiles?.full_name || b.profiles?.email?.split('@')[0] || 'Client';
    const date = b.scheduled_date ? new Date(b.scheduled_date+'T00:00:00').toLocaleDateString('en-AU',{day:'numeric',month:'short'}) : '—';
    const st = b.status || 'pending';
...
async function confirmCancel(){
  ...
  await sb.from('bookings').update({status:'cancelled', mechanic_notes: fullReason}).eq('id',id);
  const b = allBookings.find(x=>x.id===id);
  if(b){ b.status='cancelled'; b.mechanic_notes=fullReason; }
...
function toggleNotifPanel(){
  ...
  if(notifPanelOpen){ unreadCount = 0; updateNotifBadge(); loadRecentNotifications(); }
}

document.addEventListener('click', e => {
  if(!document.getElementById('notif-btn')?.contains(e.target) && !document.getElementById('notif-panel')?.contains(e.target)){
    const panel = document.getElementById('notif-panel');
...
function updateNotifBadge(){
  ...
  if(cnt){ cnt.style.display = unreadCount > 0 ? 'block' : 'none'; cnt.textContent = unreadCount > 9 ? '9+' : unreadCount; }
  const sb2 = document.querySelector('#sidebar .sb-badge');
  if(sb2){ sb2.textContent = unreadCount > 0 ? unreadCount : ''; sb2.style.display = unreadCount > 0 ? '' : 'none'; }
}

async function loadRecentNotifications(){
  const since = new Date(Date.now() - 24*60*60*1000).toISOString();
  const { data } = await sb.from('bookings').select('*, profiles(full_name,email)').gte('created_at', since).order('created_at',{ascending:false}).limit(20);
  const list = document.getElementById('notif-list');
  if(!list) return;
  if(!data?.length){ list.innerHTML = '<div style="padding:20px;text-align:center;color:var(--mgray);font-size:13px">No new bookings in the last 24h</div>'; return; }
  const stColors = {pending:'#D97706',confirmed:'#059669',enroute:'#1848C8',completed:'#6B7280',cancelled:'#DC2626'};
  list.innerHTML = data.map(b=>{
    const name = b.client_name || b.profiles?.full_name || b.profiles?.email?.split('@')[0] || 'Client';
    const time = new Date(b.created_at).toLocaleTimeString('en-AU',{hour:'numeric',minute:'2-digit',hour12:true});
    const st = b.status||'pending';
...
function prependNotification(b){
  const list = document.getElementById('notif-list');
  if(!list) return;
  const time = new Date(b.created_at||Date.now()).toLocaleTimeString('en-AU',{hour:'numeric',minute:'2-digit',hour12:true});
  const div = document.createElement('div');
  div.style.cssText = 'padding:10px 12px;border-radius:8px;margin-bottom:4px;background:#EEF3FC;border-left:3px solid #1848C8;animation:fadeSlideIn .3s';
...
async function loadMechStats(vanNum){
  ...
  const { data } = await sb.from('bookings')
    ...
    .eq('van_number', vanNum).eq('status','completed').gte('scheduled_date', weekAgo);
  return data || [];
}
...
async function renderMechStats(){
  ...
  statsDiv.innerHTML = `<div class="card" style="margin-top:0">
    ...
    ${vanNums.map((v,i)=>{
      ...
      const avgRating = ratings.length ? (ratings.reduce((s,b)=>s+(b.client_rating||0),0)/ratings.length).toFixed(1) : '—';
      const maxSlots = 7*9;
      const util = Math.round((totalJobs/maxSlots)*100);
      const byDay = {};
      days7.forEach(d=>byDay[d]=0);
      data.forEach(b=>{ if(byDay[b.scheduled_date]!==undefined) byDay[b.scheduled_date]++; });
      ...
      return `<div style="background:var(--off);border-radius:10px;padding:16px">
        ...
        <div style="display:flex;align-items:flex-end;gap:3px;height:60px">
          ${days7.map((d,di)=>{
            ...
            const day=new Date(d+'T00:00:00').toLocaleDateString('en-AU',{weekday:'short'}).slice(0,2);
...
async function loadClients(){
  ...
  if(!data || data.length === 0){
    grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;color:var(--mgray);padding:48px;font-size:14px">No clients yet — they will appear here when they sign up.</div>';
    return;
  }
  ...
  if(data && data.length > 0){
    ...
    data.forEach(row=>{
      if(!grouped[row.van_number]){
        ...
        grouped[row.van_number] = {id:row.van_number,name:'Van '+row.van_number,color:colors[row.van_number]||'#1848C8',suburbs:[],driverName:''};
      }
...
async function saveVanZone(vanId){
  ...
  if(van.suburbs.length > 0){
    await sb.from('van_zones').insert(van.suburbs.map(s=>({van_number:vanId,suburb:s,postcode:'',active:true})));
  }
...
async function saveDriverName(vanId, name){
  ...
  if(name.trim()){
    await sb.from('van_zones').insert({van_number: vanId, suburb: '__driver__', postcode: name.trim(), active: true});
  }
  const van = vanZones.find(v=>v.id===vanId);
...
function addSuburb(vanId){
  ...
  if(!suburb) return;
  const van = vanZones.find(v=>v.id===vanId);
  if(!van||van.suburbs.includes(suburb)){showToast('Already added');return;}
...
function addVan(){
  ...
  const colors={1:'#1848C8',2:'#D97706',3:'#7C3AED',4:'#DC2626'};
  vanZones.push({id:newId,name:'Van '+newId,color:colors[newId]||'#6B7280',suburbs:[]});
  renderVanZones();
}

function removeVan(vanId){
  if(vanZones.length<=1){showToast('Need at least one van');return;}
  vanZones=vanZones.filter(v=>v.id!==vanId);
  renderVanZones();
}

// ── CHART ─────────────────────────────────────────────────────────────────────
const days=[980,1120,850,1340,1580,0,1210,1050,1420,1680,1240,0,1380,1520,980,1620,1750,0,1290,1440,1180,1560,1830,0,1120,890];
const max=Math.max(...days);
const chart=document.getElementById('chart');
if(chart) chart.innerHTML=days.map((v,i)=>`<div class="bar-wrap"><div class="bar" style="height:${v?Math.max((v/max)*100,4):4}%;background:${v?'#1848C8':'#F3F4F6'};opacity:${v?1:0.3}" title="$${v}"></div>${i%7===0?`<div class="bar-label">${['W1','W2','W3','W4'][Math.floor(i/7)]}</div>`:'<div class="bar-label"></div>'}</div>`).join('');

// ── ESCALATION CONTACTS ───────────────────────────────────────────────────────
async function loadContacts(){
  const { data } = await sb.from('escalation_contacts').select('*').order('role').order('first_name');
  const list = document.getElementById('contacts-list');
  if(!list) return;
  if(!data || data.length === 0){
    list.innerHTML = '<div style="text-align:center;color:var(--mgray);padding:48px;font-size:14px">No contacts yet. Add your first contact above.</div>';
    return;
  }
...
async function saveContact(){
  ...
  if(!fname||!lname||!phone){ showToast('Please fill all fields'); return; }
  if(id){
    await sb.from('escalation_contacts').update({first_name:fname,last_name:lname,phone,role}).eq('id',id);
    showToast('Contact updated ✓');
  } else {
    await sb.from('escalation_contacts').insert({first_name:fname,last_name:lname,phone,role,active:true});
...
async function loadInventory() {
  ...
  if(!tbody) return;
  tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;padding:40px;color:var(--mgray)">Loading...</td></tr>';
  ...
  if(error){
    ...
    console.log(`SQL: CREATE TABLE parts_inventory (id uuid PRIMARY KEY, name text, category text, stock integer, min_stock integer, cost_price numeric, created_at timestamptz)`);
    return;
  }
...
function renderInventory() {
  ...
  const catColors = { Brakes:'#EEF3FC', Drivetrain:'#ECFDF5', Tyres:'#FEF9C3', Cables:'#FEF3C7', Lubrication:'#F0FDF4', General:'#F3F4F6' };
  const catText   = { Brakes:'#1848C8', Drivetrain:'#059669', Tyres:'#92400E', Cables:'#D97706', Lubrication:'#15803D', General:'#6B7280' };
...
async function loadCalendar() {
  ...
  const dateTo   = new Date(new Date(calWeekStart).setDate(calWeekStart.getDate() + days - 1)).toISOString().split('T')[0];

  const title = document.getElementById('cal-title');
  if(title){
    if(calView === 'week') {
      title.textContent = calWeekStart.toLocaleDateString('en-AU',{day:'numeric',month:'short'}) + ' – ' +
        new Date(new Date(calWeekStart).setDate(calWeekStart.getDate()+6)).toLocaleDateString('en-AU',{day:'numeric',month:'short',year:'numeric'});
    } else {
      title.textContent = calWeekStart.toLocaleDateString('en-AU',{weekday:'long',day:'numeric',month:'long',year:'numeric'});
    }
  }
...
function openAdminChat(bookingId, clientName) {
  ...
  adminChatChannel = sb.channel('admin-chat-' + bookingId)
...
function closeAdminChat() {
  ...
  if(adminChatChannel){ sb.removeChannel(adminChatChannel); adminChatChannel = null; }
...
async function loadAdminChatMessages(bookingId) {
  ...
  if(!data?.length){
    msgs.innerHTML = '<div style="text-align:center;color:var(--mgray);font-size:13px;padding:20px">No messages yet. Send one to start the conversation.</div>';
    return;
  }
  msgs.innerHTML = '';
...
async function sendAdminMsg() {
  ...
  await sb.from('job_messages').insert({
...
// ── SETTINGS ──────────────────────────────────────────────────────────────────
const SETTING_KEYS = ['__biz_name__','__biz_phone__','__biz_email__','__biz_abn__','__whatsapp__'];

async function loadSettings() {
  const { data: rows } = await sb.from('van_zones')
    .select('suburb,postcode')
    .eq('van_number', 0)
    .in('suburb', SETTING_KEYS);

  const map = {};
  (rows || []).forEach(r => { map[r.suburb] = r.postcode; });
  ...
  if (map['__whatsapp__']) {
    ...
    if(twilioEl){ twilioEl.textContent = '✓ Active'; twilioEl.style.color = '#059669'; }
  } else {
    if(waStatus) {
    ...
    const twilioEl = document.getElementById('integ-twilio');
    if(twilioEl){ twilioEl.textContent = 'SMS only (no WhatsApp number)'; twilioEl.style.color = '#F59E0B'; }
  }
...
async function saveWhatsappNumber() {
  ...
  const normalized = raw.startsWith('+') ? raw : (raw.startsWith('0') ? '+61' + raw.slice(1) : '+61' + raw);
  if (!/^\+61\d{9}$/.test(normalized)) { showToast('Enter a valid AU mobile number (+61XXXXXXXXX)'); return; }

  const { error } = await sb.from('van_zones').upsert(
  ...
  const waStatus = document.getElementById('wa-status');
  if(waStatus){ waStatus.textContent = `Active ✓ — messages sent from ${normalized}`; waStatus.style.color = '#059669'; }
  const twilioEl = document.getElementById('integ-twilio');
  if(twilioEl){ twilioEl.textContent = '✓ Active'; twilioEl.style.color = '#059669'; }
...
async function checkTwilioStatus() {
  ...
  try {
    ...
    if (res.status === 400 || res.status === 200) {
      ...
      const { data: waRow } = await sb.from('van_zones').select('postcode').eq('suburb','__whatsapp__').eq('van_number',0).maybeSingle();
      if (waRow?.postcode) {
...
async function loadNotifNumbers() {
  ...
  if(!list) return;
  list.innerHTML = '<div style="text-align:center;color:var(--mgray);padding:20px;font-size:13px">Loading…</div>';

  const { data, error } = await sb.from('escalation_contacts').select('*').order('role').order('first_name');
  if(error || !data || data.length === 0){
    list.innerHTML = '<div style="text-align:center;color:var(--mgray);padding:24px;font-size:13px">No numbers yet — add your first one above.</div>';
    return;
  }
...
async function saveNotifNumber() {
  ...
  if(!fullName || !phone) { showToast('Name and phone are required'); return; }
...
setTimeout(() => { try { loadBookings(); } catch(e){} }, 2000);

// Inject notification panel into body
(function(){
  const panel = document.createElement('div');
  panel.id = 'notif-panel';
  panel.style.cssText = 'display:none;position:fixed;top:68px;right:16px;width:360px;max-height:480px;overflow-y:auto;background:var(--white);border:1px solid var(--border);border-radius:16px;box-shadow:0 8px 40px rgba(0,0,0,0.18);z-index:200';
