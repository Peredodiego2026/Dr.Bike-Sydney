...
async function renderMyBikes() {
  ...
  screen.innerHTML = `
    ${createHeader('My Bikes', true, '#profile')}
    <div style="padding:16px">
      <div id="bikes-list"><div style="text-align:center;padding:40px 0;color:var(--color-text-secondary)">Loading your bikes...</div></div>
      <button id="add-bike-btn" class="btn btn--primary btn--full" style="margin-top:16px">+ Add a Bike</button>
    </div>
    <div id="add-bike-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:200;overflow-y:auto">
      <div style="background:var(--color-bg);border-radius:16px 16px 0 0;padding:24px;position:absolute;bottom:0;left:0;right:0">
        <h3 style="font-size:17px;font-weight:700;margin-bottom:16px">Add a Bike</h3>
        <input id="bike-nickname" type="text" placeholder="Nickname (e.g. Red Trek)" style="width:100%;border:1px solid var(--color-border);border-radius:8px;padding:10px 12px;font-size:14px;background:var(--color-surface);color:var(--color-text);margin-bottom:10px">
        <input id="bike-brand" type="text" placeholder="Brand (e.g. Trek, Giant)" style="width:100%;border:1px solid var(--color-border);border-radius:8px;padding:10px 12px;font-size:14px;background:var(--color-surface);color:var(--color-text);margin-bottom:10px">
        <input id="bike-model" type="text" placeholder="Model (optional)" style="width:100%;border:1px solid var(--color-border);border-radius:8px;padding:10px 12px;font-size:14px;background:var(--color-surface);color:var(--color-text);margin-bottom:10px">
        <select id="bike-type" style="width:100%;border:1px solid var(--color-border);border-radius:8px;padding:10px 12px;font-size:14px;background:var(--color-surface);color:var(--color-text);margin-bottom:16px">
          <option value="">Bike type (optional)</option>
  ...
  async function loadBikes() {
    const { data: bikes } = await sb.from('bikes').select('*').eq('client_id', user.id).order('created_at');
    const list = screen.querySelector('#bikes-list');
    if (!bikes?.length) {
      list.innerHTML = '<div style="text-align:center;padding:40px 16px"><div style="font-size:32px;margin-bottom:12px">🚲</div><p style="color:var(--color-text-secondary);font-size:14px">No bikes added yet.<br>Add your bike to track service history.</p></div>';
      return;
    }
    list.innerHTML = bikes.map(b => `
      <div style="background:var(--color-surface);border:1px solid var(--color-border);border-radius:12px;padding:16px;margin-bottom:10px;display:flex;align-items:center;justify-content:space-between">
        <div style="display:flex;align-items:center;gap:12px">
          <div style="font-size:28px">🚲</div>
          <div>
            <div style="font-weight:700;font-size:14px">${b.nickname}</div>
            <div style="font-size:12px;color:var(--color-text-secondary)">${[b.brand,b.model].filter(Boolean).join(' ') || 'Unknown'}${b.bike_type?' · '+b.bike_type:''}</div>
          </div>
        </div>
        <button onclick="deleteBike('${b.id}')" style="background:none;border:none;color:var(--color-error,#DC2626);font-size:18px;cursor:pointer">🗑</button>
      </div>`).join('');
  }
  ...
  screen.querySelector('#add-bike-btn').addEventListener('click', () => {
    screen.querySelector('#add-bike-modal').style.display = 'block';
  });
  screen.querySelector('#cancel-bike-btn').addEventListener('click', () => {
    screen.querySelector('#add-bike-modal').style.display = 'none';
  });
  screen.querySelector('#save-bike-btn').addEventListener('click', async () => {
    const nickname = screen.querySelector('#bike-nickname').value.trim() || 'My Bike';
    const brand    = screen.querySelector('#bike-brand').value.trim();
    const model    = screen.querySelector('#bike-model').value.trim();
    const bike_type = screen.querySelector('#bike-type').value;
    const { error } = await sb.from('bikes').insert({ client_id: user.id, nickname, brand, model, bike_type });
    if (error) { showToast('Could not save bike', 'error'); return; }
    screen.querySelector('#add-bike-modal').style.display = 'none';
